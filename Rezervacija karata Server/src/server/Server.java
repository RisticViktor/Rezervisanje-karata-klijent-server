...
//		Ulazno-izlazni tokovi
	...
	private boolean ispravanEmail(String email) {
		if(email.contains("@") && email.lastIndexOf(".") > email.indexOf("@") && (email.length() - email.lastIndexOf(".") )>= 2)
			return true;
		else
			return false;
	}
		...
		try {
			...
			try {
				while(true) {
					...
					if(rok != null) {
						if(rok.getTipKarte().equals(TipKarte.OBICNA_KARTA) && !rezervacijeObicne.contains(rok))
							rezervacijeObicne.add(rok);
						if(rok.getTipKarte().equals(TipKarte.VIP_KARTA) && !rezervacijeVIP.contains(rok))
